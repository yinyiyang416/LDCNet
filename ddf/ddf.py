    ...
    def forward(self, x, joint_x=None):
        ...
        for s_b, c_b in zip(self.spatial_branch, self.channel_branch):
            ...
            o = ddf(_x, channel_filter, spatial_filter,
                    self.kernel_size, self.dilation, 1, self.kernel_combine).type_as(x)
...
class DDFBlock(nn.Module):
    def __init__(self, in_channels, kernel_size=3, stride=1, dilation=1, head=1,
                 se_ratio=0.2, nonlinearity='relu', kernel_combine='mul'):
        super(DDFBlock, self).__init__()
        out_channels = in_channels
        self.conv1 = nn.Conv2d(in_channels, in_channels, kernel_size=1, bias=False)
        self.bn1 = nn.BatchNorm2d(in_channels)
        self.relu = nn.ReLU(inplace=True)
        self.conv2 = DDFPack(in_channels, kernel_size, stride, dilation, head,
                 se_ratio, nonlinearity, kernel_combine)
