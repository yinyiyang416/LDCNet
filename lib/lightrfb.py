...
#用于测试RFB模块效果，吧RFB模块中的的多层次conv换成普通conv
class test_conv(nn.Module):
    def __init__(self, channels_in=1024, channels_mid=128, channels_out=32):
        ...
        self.br = nn.Sequential(
            BasicConv(channels_mid, channels_mid * 4, kernel_size=3, dilation=1, padding=1, groups=channels_mid, bias=False,
                      bn=True, relu=False),
            BasicConv(channels_mid * 4, channels_mid * 4, kernel_size=1, dilation=1, bias=False, bn=True, relu=True),

            BasicConv(channels_mid * 4, channels_mid * 4, kernel_size=3, dilation=1, padding=1, groups=channels_mid, bias=False,
                      relu=False),
        )
    ...
    def forward(self, x):
        x_reduce = self.reduce(self.global_se(x))
        x_mid = self.br(x_reduce)
        # print(x_mid.shape)
        # print(x.shape)
        out = self.point_global(torch.cat([x, x_mid], dim=1))
        return out
